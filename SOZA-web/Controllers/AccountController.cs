    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                default:
                    ModelState.AddModelError("", "Niepoprawne dane logowania. Spróbuj ponownie.");
                    return View(model);
            }
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var androidToken = model.AndroidToken;
                var androidHelper = new AndroidHelper(ApplicationDbContext.Create());
                if (androidHelper.IsValidToken(androidToken))
                {
                    //Complex type are required to be initiliazed (or it will throw non-nullable variable is null error)
                    ...
                    if (result.Succeeded)
                    {
                        //Non required anymore, but I leave that just in case
                ...
                {
                    AddErrors(IdentityResult.Failed("Podany token nie istnieje lub jest już zajęty przez innego użytkownika."));
                }
            }
